...
const HintGroup = ({
  ...
  <div className="flex flex-col gap-0.5">
    <span className="text-[9px] font-bold text-[#565f89] uppercase tracking-wider">
      {label}
    </span>
...
const App = ({ guestMode = false }: { guestMode?: boolean }) => {
  ...
  const queueSaveTopic = useCallback(
    (nextTopic: Topic) => {
      ...
      saveTimersRef.current[normalized.id] = window.setTimeout(async () => {
        try {
          setPersistStatus({ state: "saving", message: "Saving topic…" });
          const payload = saveTopicRequestSchema.parse({
            title: normalized.title,
            topic: normalized,
          });
          ...
          const storedSerialized = stableStringify(
            data.data.topic.topic ?? null,
          );
          if (storedSerialized !== serialized) {
            setPersistStatus({
              state: "mismatch",
              message: "Saved, but verification mismatch (JSON order/shape).",
            });
            console.warn("Persistence mismatch", {
              expected: normalized,
              actual: data.data.topic.topic,
            });
            return;
          }
    ...
    [formatPersistError, isHydrated, useLocalPersistence, userId],
  );
  ...
  const deletePersistedTopic = async (topicId: string) => {
    ...
    try {
      const response = await fetch(
        `/api/content?id=${encodeURIComponent(topicId)}`,
        {
          method: "DELETE",
        },
      );
  ...
  const openTopic = (id: string) => {
    ...
    const draftFolder = folderDrafts[id] ?? target.folder;
    const resolvedTopic =
      draftTitle === target.title && draftFolder === target.folder
        ? target
        : { ...target, title: draftTitle, folder: draftFolder };
    setTopics((prev) =>
      prev.map((item) => (item.id === id ? resolvedTopic : item)),
    );
    setActiveTopicId(id);
    syncTopicMetaDraft(id, {
      title: resolvedTopic.title,
      folder: resolvedTopic.folder,
    });
    resetHistory({ topic: resolvedTopic, cursorIdx: 0, derivIdx: -1 });
    setTopicMenuEditingTarget(null);
    setIsDocumentSwitcherOpen(false);
  ...
  const createTopic = () => {
    ...
    setTopics((prev) => [...prev, newTopic]);
    setActiveTopicId(newTopic.id);
    syncTopicMetaDraft(newTopic.id, {
      title: newTopic.title,
      folder: newTopic.folder,
    });
    resetHistory({ topic: newTopic, cursorIdx: 0, derivIdx: -1 });
    setTopicMenuIndex(Math.max(0, topics.length));
    setTopicMenuEditingTarget({ id: newTopic.id, field: "title" });
  ...
  const commitTopicMetaDraft = (
    ...
    const shouldCommitTitle = !fields || fields.includes("title");
    const shouldCommitFolder = !fields || fields.includes("folder");
    const nextTitle = shouldCommitTitle
      ? (topicDrafts[id] ?? target.title)
      : target.title;
    const nextFolder = shouldCommitFolder
      ? (folderDrafts[id] ?? target.folder)
      : target.folder;
    if (nextTitle === target.title && nextFolder === target.folder) return;
    const updated = {
      ...target,
  ...
  const deleteTopic = (id: string) => {
    setTopics((prev) => {
      ...
      const nextActive =
        id === activeTopicId
          ? nextTopics[0]
          : nextTopics.find((item) => item.id === activeTopicId) ||
            nextTopics[0];
      if (nextActive.id !== activeTopicId) {
        setActiveTopicId(nextActive.id);
        syncTopicMetaDraft(nextActive.id, {
          title: nextActive.title,
          folder: nextActive.folder,
        });
        resetHistory({ topic: nextActive, cursorIdx: 0, derivIdx: -1 });
      }
      setTopicMenuIndex((prevIndex) =>
        Math.min(prevIndex, Math.max(0, nextTopics.length - 1)),
      );
      return nextTopics;
    });
    setTopicDrafts((prev) => {
              ...
              window.setTimeout(() => {
                document
                  .querySelector<HTMLInputElement>(
                    `[data-testid="topic-title-input-${target.id}"]`,
                  )
                  ?.focus();
              }, 0);
            }
          ...
          if (e.key === "f") {
            ...
            if (target) {
              ...
              window.setTimeout(() => {
                document
                  .querySelector<HTMLInputElement>(
                    `[data-testid="topic-folder-input-${target.id}"]`,
                  )
                  ?.focus();
              }, 0);
            }
    ...
    const hydrateTopics = async () => {
      ...
      if (useLocalPersistence) {
        ...
        const normalized =
          stored && stored.length
            ...
            : [createEmptyTopic("Untitled Topic")];
        const nextActive =
          normalized.find((item) => item.id === activeId) || normalized[0];
        if (!isActive) return;
        setTopics(normalized);
        setActiveTopicId(nextActive.id);
        setTopicDrafts(
        ...
        setFolderDrafts(
          normalized.reduce(
            (acc, item) => ({ ...acc, [item.id]: item.folder }),
            {},
          ),
        );
        resetHistory({ topic: nextActive, cursorIdx: 0, derivIdx: -1 });
        if (!stored || stored.length === 0)
          saveLocalTopics(normalized, nextActive.id);
        setIsHydrated(true);
        return;
      }
      if (!userId) return;
      try {
        ...
        const normalized = payload.data.topics.map((row) =>
          normalizeTopic(topicSchema.parse(row.topic)),
        );
        const nextTopics = normalized.length
          ? normalized
          : [createEmptyTopic("Untitled Topic")];
        const nextActive = nextTopics[0];
        if (!isActive) return;
  ...
  useEffect(() => {
    setTopics((prev) =>
      prev.map((item) =>
        item.id === activeTopicId ? { ...hState.topic } : item,
      ),
    );
  }, [hState.topic, activeTopicId]);
  ...
  const handleTopicMetaInputBlur = (id: string, field: "title" | "folder") => {
    ...
    window.setTimeout(() => {
      ...
      const expectedPrefix =
        field === "title" ? "topic-title-input-" : "topic-folder-input-";
      if (activeTestId === `${expectedPrefix}${id}`) return;
  ...
  return (
    <div className="flex flex-col h-screen overflow-hidden font-sans">
        ...
        <div className="flex items-center gap-3 justify-end min-w-[120px]">
          <div className="flex gap-2 items-center">
            <button
              className="h-8 w-8 rounded-full border border-[#2a2f45] bg-[#1f2335] text-[10px] font-bold text-[#c0caf5] shadow-md hover:border-[#7aa2f7]/60 hover:bg-[#24283b] transition"
              onClick={() => {
                if (!guestMode) setIsAccountOpen(true);
              }}
              aria-label="Open account settings"
            >
              {!guestMode && user?.image ? (
      ...
      <div
        className={`p-1 flex justify-between items-center text-[10px] font-bold uppercase text-[#1a1b26] transition-colors duration-200 ${getStatusColor()}`}
      >
          ...
          <div className="w-[min(920px,92vw)] max-h-[85vh] overflow-hidden rounded-2xl border border-[#22283a] bg-[#141821] shadow-[0_30px_80px_rgba(6,8,14,0.65)]">
            <div className="flex items-center justify-between border-b border-[#1f2536] bg-[#171c28] px-5 py-4">
              <div className="flex items-center gap-3">
                ...
                />
                <div>
                  <div className="text-[11px] font-bold tracking-[0.2em] text-[#cbd3f2]">
                    ACCOUNT
                  </div>
                  <div className="text-[10px] text-[#94a0c6]">
            ...
            <div className="max-h-[calc(85vh-64px)] overflow-y-auto p-5 space-y-5">
              <div className="rounded-xl border border-[#22283a] bg-[#161b27] p-4">
                ...
                <div className="mt-4 grid gap-4 md:grid-cols-[140px_1fr]">
                  <div className="flex flex-col items-start gap-3">
                    ...
                    <div className="flex flex-col gap-2">
                      <label className="text-[10px] uppercase tracking-[0.2em] text-[#7f8bb4]">
                        Avatar
                      </label>
                      <div className="flex items-center gap-2">
                        <input
                      ...
                      {avatarStatus.message && (
                        <div
                          className={`text-[10px] ${avatarStatus.type === "error" ? "text-[#ff9aaa]" : "text-[#9ece6a]"}`}
                        >
                          {avatarStatus.message}
                        </div>
                      )}
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      {profileStatus.message && (
                        <span
                          className={`text-[10px] ${profileStatus.type === "error" ? "text-[#ff9aaa]" : "text-[#9ece6a]"}`}
                        >
                          {profileStatus.message}
                        </span>
                      )}
                ...
                <div className="mt-4 space-y-3">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    {passwordStatus.message && (
                      <span
                        className={`text-[10px] ${passwordStatus.type === "error" ? "text-[#ff9aaa]" : "text-[#9ece6a]"}`}
                      >
                        {passwordStatus.message}
                      </span>
                    )}
              ...
              <div className="rounded-xl border border-[#22283a] bg-[#161b27] p-4">
                ...
                <div className="mt-2 flex items-center justify-between">
                  <div>
                    <div className="text-[10px] font-bold text-[#cbd3f2]">
                      Keybuffer legend
                    </div>
                    <div className="text-[10px] text-[#94a0c6]">
            ...
            <div className="p-5 space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <div className="mt-1 text-[10px] text-[#7aa2f7]">
                    {topic.folder || "Uncategorized"}
                  </div>
                  <div className="text-[12px] text-[#94a0c6]">
                    {topic.title}
                  </div>
                </div>
                <button
              ...
              <div className="rounded-lg border border-[#22283a] bg-[#161b27] px-3 py-2 text-[10px] text-[#94a0c6]">
                {topicMenuEditingTarget ? (
                  <span>
                    ...
                    Esc finish, Enter open
                  </span>
                ) : (
                  <span>
                    <span className="font-bold text-[#cbd3f2]">Keys:</span> j/k
                    move, o new, c edit title, f edit folder, Enter/l open, d
                    delete, Esc close
                  </span>
                )}
              </div>
              <div className="space-y-3" data-testid="topic-list">
                {orderedFolders.map((folder) => (
                  <div key={folder} className="space-y-2">
                    <div className="text-[10px] font-bold tracking-[0.08em] text-[#7aa2f7]">
                      {folder}
                    </div>
                    {groupedTopics[folder].map((item) => {
                      const index = topics.findIndex(
                        (topicItem) => topicItem.id === item.id,
                      );
                      return (
                        <div
                          key={item.id}
                        ...
                        >
                          <div className="w-full space-y-1.5">
                            <input
                              ...
                              autoFocus={
                                item.id === topicMenuEditingTarget?.id &&
                                topicMenuEditingTarget.field === "title"
                              }
                              onChange={(e) =>
                                updateTopicMeta(item.id, {
                              ...
                              onBlur={() =>
                                handleTopicMetaInputBlur(item.id, "title")
                              }
                              onKeyDown={(e) => {
                                if (e.key === "Escape") {
                                  e.preventDefault();
                            ...
                            <input
                              ...
                              autoFocus={
                                item.id === topicMenuEditingTarget?.id &&
                                topicMenuEditingTarget.field === "folder"
                              }
                              onChange={(e) =>
                                updateTopicMeta(item.id, {
                              ...
                              onBlur={() =>
                                handleTopicMetaInputBlur(item.id, "folder")
                              }
                              onKeyDown={(e) => {
                                if (e.key === "Escape") {
                                  e.preventDefault();
