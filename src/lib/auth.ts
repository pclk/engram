...
const NEON_REQUIRED_ENV_KEYS = ['VITE_NEON_AUTH_URL', 'VITE_NEON_DATA_API_URL'] as const;
...
const resolveNeonConfig = (): { config: NeonConfig | null; missingKeys: NeonRequiredEnvKey[] } => {
	const authUrl = readEnv('VITE_NEON_AUTH_URL');
	const dataApiUrl = readEnv('VITE_NEON_DATA_API_URL');
	const missingKeys = NEON_REQUIRED_ENV_KEYS.filter(key => (key === 'VITE_NEON_AUTH_URL' ? !authUrl : !dataApiUrl));
